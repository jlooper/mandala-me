<template web>
  <v-content style="padding:10px">
    <v-container grid-list-md fluid row wrap class="wrapper">
      ...
      <v-layout row wrap>
        <v-flex v-for="image in images" :key="image.id" xs6 sm4 md2>
          <v-card class="card">
            <img :src="image.image" class="image" alt="image" width="200px" height="200px">
            <v-card-actions>
              <v-spacer></v-spacer>

              <v-btn v-if="favoriteImages.length <= 4" icon @click="addToFavorites(image)">
    ...
    <!--favorited-->
    <v-container v-if="favoriteImages.length > 0" grid-list-md fluid row wrap class="wrapper">
      <h1>My choice</h1>
      <v-layout row wrap>
        <v-flex xs6 sm4 md2 v-for="(image, index) in favoriteImages" :key="image.id">
