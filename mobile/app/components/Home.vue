<template>
    ...
    <MultiDrawer>
            ...
            <Image
                ...
                stretch="aspectFit"
            />
            <Button text="Upload" width="50%" :visibility="paintingImage ? 'visible' : 'collapsed'" class="btn btn-primary btn-rounded-sm" @tap="upload()"/>
        ...
        <StackLayout>
            ...
            <GridLayout rows="80,auto,*" slot="default" class="card">
                <Image row="0" src="~/images/mini-mandala.png" class="mini-mandala"/>
...
export default {
  ...
  methods: {
    ...
    upload(){
        ...
        let fullPath = this.getFilename(localPath);
        this.paintingImage.saveToFile(localPath, "png");
        this.uploadFile(localPath).then((uploadedFile) => {
          console.log('uploaded file ', uploadedFile)
          this.getDownloadUrl(uploadedFile.name).then((downloadUrl) => {
            if (downloadUrl) {
                this.addToDb(downloadUrl)
                this.paintingImage = '';
